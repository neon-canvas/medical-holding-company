<?php
/**
* Template Name: Homepage
* @package Canvas
**/
get_header(); ?>

<main>
	<section class="welcome">
		<div class="container-sml">
			<h1 class="small">Memphis Obstetrics & Gynecological Association, P.C.</h1>
			<p class="large">A Clinically Integrated Medical Practice</p>
			<p style="text-align: left;">Memphis Obstetrics & Gynecological Association, P.C. is a clinically integrated medical practice, providing obstetrical, gynecological, urogynecological and diagnostic services to women in Memphis and the Mid-South.</p>

			<p style="text-align: left;">Founded in 1985, Memphis Obstetrical & Gynecological Assn., P.C. has grown by merging in established medical groups to further enhance the clinical services provided to each group’s patients. Increasing the number of clinicians in a practice allows the practice to provide more services, and to establish clinical guidelines for the clinicians to follow thus enhancing the quality of the practice.</p>

			<p style="text-align: left;">Please see the information below describing our divisions. To learn more, click on the link provided.</p>
		</div>
	</section>
	<section class="info">
		<div class="container">
			<h2 style="margin: 0 0 30px;">Memphis Obstetrics & Gynecological Association, P.C.</h2>
			<table class="doctors-table">
				<thead>
					<tr>
						<th scope="col">
							<a href="http://www.mogamd.com" target="_blank"><img src="/wp-content/themes/easel-v4/img/moga-logo.jpg" alt="MOGA logo"></a>
							<span><a href="http://www.mogamd.com" target="_blank">www.mogamd.com</a></span>
						</th>
						<th scope="col">
							<a href="http://www.mestemd.com" target="_blank"><img src="/wp-content/themes/easel-v4/img/mcw-logo.jpg" alt="MCW logo"></a>
							<span><a href="http://www.mestemd.com" target="_blank">www.mestemd.com</a></span>
						</th>
						<th scope="col">
							<a href="http://www.wpgmd.com" target="_blank"><img src="/wp-content/themes/easel-v4/img/wpg-logo.jpg" alt="WPG logo"></a>
							<span><a href="http://www.wpgmd.com" target="_blank">www.wpgmd.com</a></span>
						</th>
						<th scope="col">
							<a href="http://www.whsobgyn.com" target="_blank"><img src="/wp-content/themes/easel-v4/img/whs-logo.jpg" alt="WHS logo"></a>
							<span><a href="http://www.whsobgyn.com" target="_blank">www.whsobgyn.com</a></span>
						</th>
					</tr>
				</thead>

				<tbody>
					<tr>
						<td>John F. Albritton, M.D.</td>
						<td>Christine Mestemacher, M.D.</td>
						<td>Lanetta Anderson, M.D.</td>
						<td>Vanessa Givens, M.D.</td>
					</tr>
					<tr>
						<td>Lea Mary Bannister, M.D.</td>
						<td>Yvonne Moore, M.D.</td>
						<td>Belvia Carter, M.D.</td>
						<td>Sherri Li, M.D.</td>
					</tr>
					<tr>
						<td>Linda W. Childers, MSN, CFNP</td>
						<td>Leslie Norman, MSN, FNP</td>
						<td>Teri-Lee Foster, M.D.</td>
						<td>Frank Ling, M.D.</td>
					</tr>
					<tr>
						<td>Crista L. Crisler, M.D.</td>
						<td></td>
						<td>Deidrea Grandberry, M.D.</td>
						<td>Amy Nelson, M.D.</td>
					</tr>
					<tr>
						<td>Jennifer Dewey, MSN, FNP</td>
						<td></td>
						<td>Dionne Rouselle, M.D.</td>
						<td>Thomas Stovall, M.D.</td>
					</tr>
					<tr>
						<td>Katherine Dorsett, M.D.</td>
						<td></td>
						<td>Carlisha Whitlow, MSN, FNP</td>
						<td>Margaret Summitt, M.D.</td>
					</tr>
					<tr>
						<td>Kristen Duncan, WHNP</td>
						<td></td>
						<td></td>
						<td>Robert Summitt, M.D.</td>
					</tr>

					<tr>
						<td>G. Aric Giddens, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>M. Andrea Giddens, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Haley Gore, MSN, FNP-C</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>
					
					<tr>
						<td>Lynn G. Kirkland, DNSc, WHNP</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Alok Kumar, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Daniel A. Lee, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Fazal M. Manejwala, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Elizabeth K. Mann, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>J. Elizabeth McAdory, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Jason B. Mullenix, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Kelly K. Pfrommer, MSN, FNP</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Helena G. Shannon, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Elaine A. Thompson, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Heather M. Wherry, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Jason G. Williams, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>James A. Wilson, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>

					<tr>
						<td>Alicia W. Wright, M.D.</td>
						<td></td>
						<td></td>
						<td></td>
					</tr>
				</tbody>
			</table>

			<div class="mobile-doctor-list">
				<img src="/wp-content/themes/easel-v4/img/moga-logo.jpg" alt="MOGA logo">
				<a href="http://www.mogamd.com" target="_blank">www.mogamd.com</a>
				<ul class="dr-list">
					<li>John F. Albritton, M.D.</li>
					<li>Lea Mary Bannister, M.D.</li>
					<li>Linda W. Childers, MSN, CFNP</li>
					<li>Crista L. Crisler, M.D.</li>
					<li>Jennifer Dewey, MSN, FNP</li>
					<li>Katherine Dorsett, M.D.</li>
					<li>Kristen Duncan, WHNP</li>
					<li>G. Aric Giddens, M.D.</li>
					<li>M. Andrea Giddens, M.D.</li>
					<li>Haley Gore, MSN, FNP-C</li>
					<li>Lynn G. Kirkland, DNSc, WHNP</li>
					<li>Alok Kumar, M.D.</li>
					<li>Daniel A. Lee, M.D.</li>
					<li>Fazal M. Manejwala, M.D.</li>
					<li>Elizabeth K. Mann, M.D.</li>
					<li>J. Elizabeth McAdory, M.D.</li>
					<li>Jason B. Mullenix, M.D.</li>
					<li>Kelly K. Pfrommer, MSN, FNP</li>
					<li>Helena G. Shannon, M.D.</li>
					<li>Elaine A. Thompson, M.D.</li>
					<li>Heather M. Wherry, M.D.</li>
					<li>Jason G. Williams, M.D.</li>
					<li>James A. Wilson, M.D.</li>
					<li>Alicia W. Wright, M.D.</li>
				</ul>
			</div>

			<div class="mobile-doctor-list">
				<img src="/wp-content/themes/easel-v4/img/mcw-logo.jpg" alt="MCW logo">
				<a href="http://www.mestemd.com" target="_blank">www.mestemd.com</a>
				<ul class="dr-list">
					<li>Christine Mestemacher, M.D.</li>
					<li>Yvonne Moore, M.D.</li>
					<li>Leslie Norman, MSN, FNP</li>
				</ul>
			</div>

			<div class="mobile-doctor-list">
				<img src="/wp-content/themes/easel-v4/img/wpg-logo.jpg" alt="WPG logo">
				<a href="http://www.mogamd.com" target="_blank">www.wpgmd.com</a>
				<ul class="dr-list">
					<li>Lanetta Anderson, M.D.</li>
					<li>Belvia Carter, M.D.</li>
					<li>Teri-Lee Foster, M.D.</li>
					<li>Deidrea Grandberry, M.D.</li>
					<li>Dionne Rouselle, M.D.</li>
					<li>Carlisha Whitlow, MSN, FNP</li>
				</ul>
			</div>

			<div class="mobile-doctor-list">
				<img src="/wp-content/themes/easel-v4/img/whs-logo.jpg" alt="WHS logo">
				<a href="http://www.mogamd.com" target="_blank">www.whsobgyn.com</a>
				<ul class="dr-list">
					<li>Vanessa Givens, M.D.</li>
					<li>Sherri Li, M.D.</li>
					<li>Frank Ling, M.D.</li>
					<li>Amy Nelson, M.D.</li>
					<li>Thomas Stovall, M.D.</li>
					<li>Margaret Summitt, M.D.</li>
					<li>Robert Summitt, M.D.</li>
				</ul>
			</div>

		</div>
	</section>
</main>

<?php get_footer(); ?>
