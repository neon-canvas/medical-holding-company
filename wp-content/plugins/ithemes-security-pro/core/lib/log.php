...
final class ITSEC_Log {
	...
	private static function add_to_db( $data ) {
		...
		if ( false === $result ) {
			...
			return new WP_Error( 'itsec-log-failed-db-insert', sprintf( esc_html__( 'Failed to insert log entry: %s', 'it-l10n-ithemes-security-pro' ), $wpdb->last_error ) );
		}
	...
	private static function add_to_file( $data, $id = false ) {
		...
		if ( false === $result ) {
			return new WP_Error( 'itsec-log-failed-to-write-to-file', __( 'Unable to write to the log file. This could indicate that there is no space available, that there is a permissions issue, or that the server is not configured properly.', 'it-l10n-ithemes-security-pro' ) );
		}
